...
$(document).ready(function(){
	...
	if($('#rate-calc-form').length){
		...
		$rc_form.find('#submit_btn, .submit_question').on('click', function(){
			autoscroll = true;
			if($rc_form.find('input#answer').length){
				$rc_form.find('input#answer').val($(this).val());
			}
		});
	...
	if($('aside').find('.sticky-block').length){
		...
		$(document).on('click', '.page-content a[href^="#"]', function(e){
			//e.preventDefault();
			$('html, body').stop().animate({
	...
	var ajaxGetPartialsContent = function($container, view_file){
	...
	if($('.ajaxContent').length){
		setTimeout(function(){
			$('.ajaxContent').each(function(){
				...
				ajaxGetPartialsContent($(this), view_file);
			});
		}, 3000);
	}
