...
class ETClient extends Component{
	...
	public function get_quotes($args = []){
		...
		if($update){
			...
			if(intval($data['status']) == 200 && $data['response'] != null){
	...
	private function appendAdditionalFields($args, $data){
		foreach($data['response']['quotes'] as $k => $v){
			...
			$data['response']['quotes'][$k]['company_code']    = 'ethos';
			$data['response']['quotes'][$k]['company_name']    = 'Ethos';
			$data['response']['quotes'][$k]['product_code']    = 'ethos_term';
			$data['response']['quotes'][$k]['product_name']    = 'ethosTerm';
			$data['response']['quotes'][$k]['logo_url']        = '/img/company_logos/logo_ethos.png';
